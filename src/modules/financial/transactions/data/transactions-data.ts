...
export const columnCategories = {
  "Transaction Information": [
    ...
    {
      key: "calculatedConversionRate",
      label: "Calculated Conversion Rate",
      type: "number",
    },
  ],
  Account: [
  ...
  "PSP Information": [
    ...
    {
      key: "pspChildTransactionId",
      label: "PSP Child Transaction ID",
      type: "string",
    },
  ...
  "Additional Information": [
    ...
    {
      key: "transactionImportId",
      label: "Transaction Import ID",
      type: "string",
    },
...
export const transactionsData: Transaction[] = Array.from(
  { length: 100 },
  (_, index) => {
    ...
    const pspStatus =
      pspStatuses[Math.floor(Math.random() * pspStatuses.length)];
    const name = names[Math.floor(Math.random() * names.length)];
    const paymentSystem =
      paymentSystems[Math.floor(Math.random() * paymentSystems.length)];
    ...
    return {
      ...
      amount,
      amountUsd: Math.floor(
        amount * (currency === "USD" ? 1 : Math.random() * 1.5 + 0.5)
      ),
      ...
      calculatedConversionRate:
        currency === "USD" ? 1 : Math.random() * 1.5 + 0.5,
      checkedAt: processedAt,
      checkedBy: "System",
      client: name,
      clientIp: `192.168.${Math.floor(Math.random() * 255)}.${Math.floor(
        Math.random() * 255
      )}`,
      ...
      transferInOutId: type.includes("transfer")
